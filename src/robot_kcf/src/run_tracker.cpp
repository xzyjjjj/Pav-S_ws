...
void ImageConverter::yoloCb(const std::shared_ptr<vision_msgs::msg::Detection2DArray> msg)
{
  const auto& det = msg->detections[0];
  bool no_bbox = (det.bbox.center.position.x == 99999)&&(det.bbox.center.position.y == 99999)&&(det.bbox.size_x == 99999)&&(det.bbox.size_y == 99999);
  // 如果没有检测框，立即停止跟踪
  if (no_bbox) {
    bBeginKCF = false;
    RCLCPP_WARN(this->get_logger(), "No YOLO detection received, stopping tracking.");
    return;
  }
/*
  // 获取当前消息的时间戳
  rclcpp::Time currentStamp = msg->header.stamp;

  // 如果时间戳没有变化，说明是旧消息，停止跟踪
  if (currentStamp == lastYoloStamp)
  {
    bBeginKCF = false;
    RCLCPP_INFO(this->get_logger(), "YOLO timestamp unchanged, stop tracking.");
    return;
  }


  // 更新时间戳
  lastYoloStamp = currentStamp;
*/
  // 获取检测框
...
void ImageConverter::depthCb(const std::shared_ptr<sensor_msgs::msg::Image> msg) {
    ...
    if (enable_get_depth) {
        ...
        if (twist.linear.x>0.5) {twist.linear.x=0.35;}

        if (twist.linear.x<-0.5) twist.linear.x=-0.35;

        if (twist.angular.z>0.5) twist.angular.z=0.35;
        if (twist.angular.z<-0.5) twist.angular.z=-0.35;
